...
/**
 * << TestVectorGenAndB2BTest.groovy >>
 *
 * This job is for running "TestVectorGenAndB2BTest" of mdl_verification process. It consists of two stages, 
 ...
 * to be executed by TestVectorGenAndB2BTest, and executes it on them. Then, upload the artifacts to the specified
...
def yamlConfig
def processName = 'mdl_verification'
def toolName = 'TestVectorGenAndB2BTest'
                ...
                script {
                    ...
                    try {
                        ...
                        for (searchResult in searchResults) {
                            ...
                            try {
                                // Execute TestVectorGenAndB2BTest
                                testVectorGenAndB2BTest(newYamlConfig.parameters)
                            } catch (CICTFrameworkException e) {
                                if (e.id.equals('0005600003')) {   // MATLAB returned nonzero exit
                                    ...
                                    if (quotientRetVal == 1) {   // Warning => UNSTABLE
                                        ...
                                        printMessage('0008700004', searchResult.fileName)
                                ...
                                fileOperations([
                                    folderCreateOperation(folderPath:"Result\\${searchResult.basename}\\tests"),
                                    folderCreateOperation(folderPath:"Result\\${searchResult.basename}\\reports")
                                ])
                                if (fileExists("${searchResult.fileDir}\\tests")) {
                                    fileOperations([
                                        folderCopyOperation(sourceFolderPath:"${searchResult.fileDir}\\tests",destinationFolderPath:"Result\\${searchResult.basename}\\tests"),
                                        folderDeleteOperation(folderPath:"${searchResult.fileDir}\\tests")
                                    ])
                                }
                                if (fileExists("${searchResult.fileDir}\\reports")) {
                                    fileOperations([
                                        folderCopyOperation(sourceFolderPath:"${searchResult.fileDir}\\reports",destinationFolderPath:"Result\\${searchResult.basename}\\reports"),
                                        folderDeleteOperation(folderPath:"${searchResult.fileDir}\\reports")
                                    ])
                                }
                            }
                        }
                        ...
                        mbdCommonProcessing.publishHTMLReport(yamlConfig, '**\\*-MilsCoverage.html,**\\TestReport.htm', '**\\*.html,**\\*.htm,**\\*.gif,**\\*.txt,**\\*.csv')
