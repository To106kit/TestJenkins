 ...
 * << StubGen.groovy >>
 *
 * This job is for running "StubGen" of mdl_coding process. It consists of two stages, 
 ...
 * to be executed by StubGen, and executes it on them. Then, upload the artifacts to the specified
...
def yamlConfig
def processName = 'mdl_coding'
def toolName = 'StubGen'
                    ...
                    try {
                        ...
                        for (searchResult in searchResults) {
                            ...
                            try {
                                //-----------------------------------------------------------------------
                                // �b�菈�u
                                // �Q�ƃ��f���̃R�[�h�ƁA�Q�ƌ����f���̃R�[�h�����킹�� StubGen �ɓn��
                                if (newYamlConfig.refmdlCodeDir != null) {
                                    if (fileExists(newYamlConfig.refmdlCodeDir)) {
                                        fileOperations([
                                            folderCopyOperation(
                                                sourceFolderPath: newYamlConfig.refmdlCodeDir,
                                                destinationFolderPath: newYamlConfig.parameters.srcdir
                                            )
                                        ])
                                    }
                                }
                                ...
                                // Execute StubGen
                                stubgen4dnec(newYamlConfig.parameters)
                            } catch (CICTFrameworkException e) {
                                if (e.id.equals('0005600003')) {   // MATLAB returned nonzero exit
                                    ...
                                    if (quotientRetVal == 1) {   // Warning => UNSTABLE
                                        ...
                                        printMessage('0008900001', searchResult.fileName)
                                ...
                                fileOperations([
                                    folderCreateOperation(folderPath:"Result\\${searchResult.basename}")
                                ])
                                if (fileExists('StubGenResult')) {
                                    fileOperations([
                                        folderCopyOperation(sourceFolderPath:'StubGenResult',destinationFolderPath:"Result\\${searchResult.basename}"),
                                        folderDeleteOperation(folderPath:'StubGenResult')
                                    ])
                                }
                            }
                        }
